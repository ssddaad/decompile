...
// 尝试加载 acorn 作为 esprima 的后备解析器
// esprima 4.0.1 不支持 ES2020+ 语法（可选链 ?.、空值合并 ?? 等），
// 而 acorn 8.x 支持最新语法，两者均产出 ESTree 兼容 AST
let acorn = null;
try { acorn = require('acorn'); } catch(e) {}

function parseScriptSafe(code) {
    try {
        return esprima.parseScript(code);
    } catch(e1) {
        if (acorn) {
            console.log("[wuWxml] esprima parse failed, falling back to acorn: " + e1.message);
...
function doWxml(state, dir, name, code, z, xPool, rDs, wxsList, moreInfo) {
    ...
    let r = {son: []};
    analyze(parseScriptSafe(code).body, z, {[rname]: r}, xPool, {[rname]: r});
    let ans = [];
...
function doFrame(name, cb, order, mainDir) {
    ...
    wxsList = {};
    console.log("[wuWxml] doFrame start: " + name);
    wu.get(name, code => {
        try {
            console.log("[wuWxml] File read: " + code.length + " bytes");
            getZ(code, z => {
                try {
                    // ---- 提取 require 系统和页面函数 ----
                    // 尝试多个起始标记
                    ...
                    let beforeStr = null, beforeIdx = -1;
                    for (const m of beforeMarkers) {
                        const idx = code.lastIndexOf(m);
                        if (idx !== -1) { beforeStr = m; beforeIdx = idx; break; }
                    }
                    if (beforeIdx === -1) {
                        throw new Error("Cannot find nv_require start marker in code");
                    }
                    if (beforeStr !== "\nvar nv_require=function(){var nnm=") {
                        console.log("[wuWxml] Using alt start marker: '" + beforeStr.slice(0, 40) + "'");
                    }
                    ...
                    let endStr = null, endIdx = -1;
                    for (const m of endMarkers) {
                        const idx = code.lastIndexOf(m);
                        if (idx !== -1) { endStr = m; endIdx = idx; break; }
                    }
                    if (endIdx === -1 || endIdx <= beforeIdx) {
                        throw new Error("Cannot find end marker or end before start");
                    }

                    let extractedCode = code.slice(beforeIdx + beforeStr.length, endIdx);
                    console.log("[wuWxml] Extracted code: " + extractedCode.length + " bytes");

                    // 提取 JSON
                    let jsonEnd = extractedCode.indexOf("};");
                    if (jsonEnd === -1) {
                        throw new Error("Cannot find '};' to extract JSON");
                    }
                    let json = extractedCode.slice(0, jsonEnd + 1);

                    // 查找 require 函数结束位置
                    let endOfRequire = extractedCode.indexOf("()\r\n") + 4;
                    if (endOfRequire == 3) endOfRequire = extractedCode.indexOf("()\n") + 3;
                    if (endOfRequire < 3) {
                        throw new Error("Cannot find end of require function");
                    }
                    let vmCode = extractedCode.slice(endOfRequire);
                    ...
                    let fullVmCode = vmCode + "\n_vmRev_([x," + json + "])";
                    console.log("[wuWxml] VM code: " + fullVmCode.length + " bytes");

                    try {
                        vm.run(fullVmCode);
                    } catch (vmErr) {
                        throw new Error("VM execution failed: " + vmErr.message);
                    }

                    let pageCount = Object.keys(rE).length;
                    let wxsCount = Object.keys(rF).length;
                    console.log("[wuWxml] VM success: " + pageCount + " pages, " + wxsCount + " WXS");

                    if (pageCount === 0) {
                        throw new Error("No pages found after VM execution (rE is empty)");
                    }

                    let dir = mainDir || path.dirname(name), pF = [];
                    // 处理 WXS 函数
                    for (let info in rF) if (typeof rF[info] == "function") {
                        let wxsName = path.resolve(dir, (info[0] == '/' ? '.' : '') + info), ref = rF[info]();
                        pF[ref] = info;
                        try {
                            wu.save(wxsName, doWxs(requireInfo[ref].toString()));
                        } catch (e) {
                            console.log("[wuWxml] WXS save error for " + info + ": " + e.message);
                        }
                    }
                    for (let info in rF) if (typeof rF[info] == "object") {
                        let wxsName = path.resolve(dir, (info[0] == '/' ? '.' : '') + info);
                        let res = [], now = rF[info];
                        for (let deps in now) {
                            let ref = now[deps]();
                            ...
                            wxsList[wxsName] = res.join("\n");
                        }
                    }

                    // 恢复每个页面的 WXML
                    let successCount = 0, failCount = 0;
                    for (let pageName in rE) {
                        try {
                            tryWxml(dir, pageName, rE[pageName].f.toString(), z, x, rD[pageName], wxsList, moreInfo);
                            successCount++;
                        } catch (e) {
                            failCount++;
                            console.error("[wuWxml] Failed to restore " + pageName + ": " + e.message);
                        }
                    }

                    console.log("[wuWxml] doFrame complete: " + successCount + " ok, " + failCount + " failed");
                    cb({[name]: 4, success: successCount, failed: failCount});
                } catch (e) {
                    console.error("[wuWxml] Error in getZ callback: " + e.message);
                    console.error(e.stack);
                    cb({error: e.message});
                }
            });
        } catch (e) {
            console.error("[wuWxml] Error in doFrame: " + e.message);
            console.error(e.stack);
            cb({error: e.message});
        }
    });
