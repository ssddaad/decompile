...
// ---- 标记搜索工具：尝试多个候选标记，返回第一个命中的位置 ----
function findMarker(code, markers) {
    for (const m of markers) {
        const idx = code.lastIndexOf(m);
        if (idx !== -1) return { idx, marker: m };
    }
    return null;
}
...
function catchZGroup(code, groupPreStr, cb) {
    ...
    for (let preStr of groupPreStr) {
        ...
        // 尝试多个起始标记
        let startIdx = content.indexOf("(function(z){var a=11;");
        if (startIdx === -1) {
            const alt = findMarker(content, [
                "(function(z){var a=11;",
                "function(z){var a=11;",
                "var a=11;function Z",
            ]);
            if (alt) {
                startIdx = alt.idx;
                console.log("[wuRestoreZ] Group: using alt start marker at " + startIdx);
            } else {
                console.error("[wuRestoreZ] Group: cannot find start marker for " + preStr.slice(0, 40));
                continue;
            }
        }
        content = content.slice(startIdx);
        // 尝试多个结束标记
        let endIdx = content.indexOf("})(__WXML_GLOBAL__.ops_cached.$gwx");
        if (endIdx === -1) {
            const alt = findMarker(content, [
                "})(__WXML_GLOBAL__.ops_cached.$gwx",
                "})(__WXML_GLOBAL__.ops_cached",
                "ops_cached.$gwx",
            ]);
            if (alt) {
                endIdx = alt.idx;
                console.log("[wuRestoreZ] Group: using alt end marker at " + endIdx);
            } else {
                console.error("[wuRestoreZ] Group: cannot find end marker for " + preStr.slice(0, 40));
                continue;
            }
        }
        content = content.slice(0, endIdx) + "})(z);";
        let vm = new VM({sandbox: {z: z, debugInfo: []}});
        try {
            vm.run(content);
        } catch (e) {
            console.error("[wuRestoreZ] Group VM error for " + preStr.slice(0, 40) + ": " + e.message);
            continue;
        }
...
function catchZ(code, cb) {
    ...
    // 尝试多个结束标记
    let lastResult = findMarker(code, [
        "(z);__WXML_GLOBAL__.ops_set.$gwx=z;",
        "(z);__WXML_GLOBAL__.ops_set.$gwx",
        "__WXML_GLOBAL__.ops_set.$gwx",
        "ops_set.$gwx",
    ]);
    if (!lastResult) {
        throw new Error("[wuRestoreZ] Cannot find ops_set.$gwx marker in code (z extraction failed)");
    }
    let lastPtr = lastResult.idx;
    if (lastResult.marker !== "(z);__WXML_GLOBAL__.ops_set.$gwx=z;") {
        console.log("[wuRestoreZ] Using alternative end marker: '" + lastResult.marker.slice(0, 40) + "'");
    }
    // 尝试多个起始标记
    let startResult = findMarker(code, [
    ...
    if (!startResult) {
        throw new Error("[wuRestoreZ] Cannot find Z(ops) start marker in code (z extraction failed)");
    }
    let startPtr = startResult.idx;
    if (startResult.marker !== '(function(z){var a=11;function Z(ops){z.push(ops)}') {
        console.log("[wuRestoreZ] Using alternative start marker: '" + startResult.marker.slice(0, 40) + "'");
    }
    if (startPtr > lastPtr) {
        throw new Error("[wuRestoreZ] Start marker is after end marker - invalid structure");
    }
    let extractedCode = code.slice(startPtr, lastPtr + 4);
    console.log("[wuRestoreZ] Extracted z code: " + extractedCode.length + " bytes");
    try {
        vm.run(extractedCode);
    } catch (vmErr) {
        throw new Error("[wuRestoreZ] VM execution failed: " + vmErr.message);
    }
    console.log("[wuRestoreZ] Z values extracted: " + z.length);
    cb(z);
}
